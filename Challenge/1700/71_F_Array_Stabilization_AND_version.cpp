...
// Link - https://codeforces.com/problemset/problem/1579/F
...
void blackout_yash() {
	...
	fora(i, 0, n) {
		if (ans[i] == -1) {
			...
			fora(i, 0, size(temp)) {
				if (flag) {
					if (!arr[temp[i]]) flag = 0;
					else index++;
				}

				if (!arr[temp[i]]) ans[temp[i]] = 0;
				else if (!flag) ans[temp[i]] = index + size(temp) - i - 1;
				else ans[temp[i]] = mm;
			}

			index = mm;
			rfora(i, 0, size(temp)) {
				if (!arr[temp[i]]) index = 0;
				else index++;

				ans[temp[i]] = min(ans[temp[i]], index);
			}
		}
	}

	int max = *max_element(all(ans));
	if (max >= mm) cout << -1;
	else cout << max;
	cout << nline;
}
