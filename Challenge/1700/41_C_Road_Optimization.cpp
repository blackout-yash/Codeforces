...
void blackout_yash() {
	...
	vector <int> dis(n), speed(n + 1);
	for (auto &xx : dis) cin >> xx;
	fora(i, 1, n + 1) cin >> speed[i];
	dis.pb(l);

	vector <vector<int>> dp(n + 1, vector<int> (k + 1, mm));
	fora(i, 0, n + 1) {
		fora(j, 0, k + 1) {
			if (!j) {
				if (!i) dp[i][j] = 0;
				else dp[i][j] = dp[i - 1][j] + (dis[i] - dis[i - 1]) * speed[i];
			} else if (i > j) {
				dp[i][j] = dp[i - 1][j] + (dis[i] - dis[i - 1]) * speed[i];
				int index = j - 1;
				rfora(k, 0, i) {
					dp[i][j] = min(dp[i][j], dp[k][index] + (dis[i] - dis[k]) * speed[k]);
					if (index) index--;
					else break;
				}
			} else break;
