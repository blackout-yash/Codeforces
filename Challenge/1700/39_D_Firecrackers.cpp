// D. Firecrackers
// Link - https://codeforces.com/contest/1468/problem/D
...
bool solve(int mid, int n, int m, int theif, int police, vector <int> &arr) {
	if (theif < police) {
		while (mid) {
			if (theif + 1 < police) {
				int _max = police - 1;
				if (_max > arr[mid - 1]) mid--;
				else break;
			} else break; police--;
		}
	} else {
...
void blackout_yash() {
	...
	int n, m, theif, police;
	cin >> n >> m >> theif >> police;
	...
	while (low <= high) {
		...
		if (solve(mid, n, m, theif, police, arr)) {
			ans = max(ans, mid);
			low = mid + 1;
		} else high = mid - 1;
	} cout << ans << nline;
