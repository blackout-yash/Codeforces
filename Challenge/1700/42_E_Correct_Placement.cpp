...
void blackout_yash() {
	...
	map <pair<int, int>, int> temp1;
	map <int, int> temp2;

	fora(i, 0, n) {
		cin >> arr[i].ff >> arr[i].ss;
		if (temp2.count(arr[i].ff)) temp2[arr[i].ff] = min(temp2[arr[i].ff], arr[i].ss);
		else temp2[arr[i].ff] = arr[i].ss;
		temp1[arr[i]] = i + 1;
	}

	vector <int> store1;
	vector <pair<int, int>> store2;
	pair<int, int> p; int min = mm;
	for (auto x : temp2) {
		if (min > x.ss) {
			min = x.ss;
			p = x;
	...
	fora(i, 0, n) {
		int index = lower_bound(all(store1), arr[i].ff) - store1.begin();
		if (index) {
			index--;
			if (arr[i].ss > store2[index].ss) {
				cout << temp1[store2[index]] << " ";
				continue;
			}
		}

		index = lower_bound(all(store1), arr[i].ss) - store1.begin();
		if (index) {
			index--;
			if (arr[i].ff > store2[index].ss) cout << temp1[store2[index]] << " ";
			else cout << -1 << " ";
		} else cout << -1 << " ";
	} cout << nline;
}
