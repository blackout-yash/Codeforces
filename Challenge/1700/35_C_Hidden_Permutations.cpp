...
// Link - https://codeforces.com/problemset/problem/1621/C
...
void blackout_yash() {
	...
	int range1 = n, range2 = 2 * n, index;
	set <int> s;
	fora(i, 1, n + 1) s.insert(i);
	...
	while (range1 && range2) {
		...
		while (range1 && range2) {
			int num = _input(index);
			temp.pb(num); s.erase(num);
			range2--;
			if (size(temp) > 1 && temp.front() == num) break;
		}

		fora(i, 1, size(temp)) ans[temp[i - 1]] = temp[i], range1--;
	}
