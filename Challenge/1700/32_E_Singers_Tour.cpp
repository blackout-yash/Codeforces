// E. Singers' Tour
// Link - https://codeforces.com/problemset/problem/1618/E
...
void blackout_yash() {
	...
	vector <int> arr(n);
	for (auto &xx : arr) cin >> xx, sum += xx;

	int size = (n * (n + 1)) / 2;
	if (sum % size || *min_element(all(arr)) < size) cout << "NO";
	else {
		int temp = sum / size;

		vector <int> ans(n, -1);
		fora(i, 0, n) {
			int temp1 = arr[(i + n - 1) % n] - (arr[i] - temp);
			if (temp1 <= 0 || temp1 % n) break;
			else ans[i] = temp1 / n;
		}

		if (*min_element(all(ans)) == -1) cout << "NO";
		else {
			cout << "YES\n";
