// C. Mixing Water
// Link - https://codeforces.com/contest/1359/problem/C
...
void blackout_yash() {
	...
	if (h + c - (2 * t)) {
		double n = (c - t) / (h + c - (2 * t));
		double n1 = floor(n), n2 = ceil(n);

		if (n1 > 0) {

			double diff1 = abs((n1 * h + (n1 - 1) * c) / (2 * n1 - 1) - t);
			double diff2 = abs((n2 * h + (n2 - 1) * c) / (2 * n2 - 1) - t);
