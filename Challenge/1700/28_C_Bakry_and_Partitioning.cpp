...
// Link - https://codeforces.com/problemset/problem/1592/C
...
int dfs(int root, int _xor, vector <vector<int>> &graph) {
	vis[root] = 1;
	...
	for (int child : graph[root]) {
		...
		ans ^= dfs(child, _xor, graph);
	}

	if (ans == _xor) cnt++, ans = 0;
	return ans;
}
...
void blackout_yash() {
	...
	int _xor = 0;
	fora(i, 0, n) cin >> arr[i + 1], _xor ^= arr[i + 1];
