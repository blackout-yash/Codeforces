...
// Link - https://codeforces.com/contest/1594/problem/D
...
/*----------------------------------------------------------------------------------------*/
const int n = 3 * 1e5;
int col[n], store[2], ans;

void dfs(int root, vector <vector<pair<int, int>>> &graph) {
	store[col[root]]++;
	for (auto x : graph[root]) {
		int child = x.ff, currCol = x.ss;

		if (col[child] == -1) {
			col[child] = (col[root] ^ currCol);
			dfs(child, graph);
		}
		else if (col[child] != (col[root] ^ currCol)) {
			ans = false;
			return;
		}
	}
...
void _reset(int n) {
	fora(i, 0, n + 1) col[i] = -1;
}


// Link - https://youtu.be/X_kVGgHcfQs?t=1212
void blackout_yash() {
	/*-------__👌Never Give Up👌__-------*/
	/*----__🤭🤭Stop Stalking🤭🤭__----*/
	...
	fora(i, 1, n + 1) {
		if (col[i] == -1) {
			...
			// col[i] = 0 or col[i] = 1, both values will give correct ans
			col[i] = 1;
			dfs(i, graph);
			cnt += max(store[0], store[1]);
		}
	}
