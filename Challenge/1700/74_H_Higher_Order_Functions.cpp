...
// Link - https://codeforces.com/problemset/problem/1578/H
...
void solve() {
	...
	int ans;
	rfora(i, 0, n - 1) {
		if (i + 2 == n) ans = max(temp[i] + 1, temp[i + 1]);
		else ans = max(temp[i] + 1, ans);
	} s.push(ans);
}
