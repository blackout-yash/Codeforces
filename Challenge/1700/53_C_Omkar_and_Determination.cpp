...
// Link - https://codeforces.com/problemset/problem/1583/C
...
void blackout_yash() {
	...
	fora(i, 0, n - 1) {
		rfora(j, 1, m) {
			if (s[i][j] == 'X' && s[i + 1][j - 1] == 'X') arr.pb(j + 1);
		}
	} sort(all(arr));
	...
	while (tt--) {
		...
		auto it = ub(all(arr), l);
		if (it == arr.end()) cout << "YES";
		else {
			int temp = arr[it - arr.begin()];
			if (l < temp && temp <= r) cout << "NO";
			else cout << "YES";
		} cout << nline;
	}
