...
// Link - https://codeforces.com/problemset/problem/1550/C
...
#ifndef ONLINE_JUDGE
#define debug(x) cerr << #x << " => "; _pprintt(x); cerr << nline;
#else
#define debug(x)
#endif
...
void blackout_yash() {
	...
	fora(i, 0, n) {
		if (i + 3 <= n) {
			int range = min(n, i + 5), _min = range;
			fora(j, i, range) {
				fora(k, j + 1, range) {
					fora(l, k + 1, range) {
						if (arr[j] >= arr[k] && arr[k] >= arr[l]) _min = min(_min, l);
						else if (arr[j] <= arr[k] && arr[k] <= arr[l]) _min = min(_min, l);
					}
				}
			} ans += (_min - i);
		} else ans += (n - i);
