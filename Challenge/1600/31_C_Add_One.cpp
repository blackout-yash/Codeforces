...
/*----------------------------------------------------------------------------------------*/
const int n = 3 * 1e5;
vector <int> store(n), factor(n);
void preCompute() {
	store[0] = 1; factor[10] = 1;
	fora(i, 1, n) {
		store[i] = store[i - 1];
		if (factor[i]) {
			store[i] = modAdd(store[i], factor[i]);
			factor[i + 9] = modAdd(factor[i + 9], factor[i]);
			factor[i + 10] = modAdd(factor[i + 10], factor[i]);
		}
	}
}
...
void solve() {
	...
	int ans = 0;
	while (n) {
		int rem = n % 10, index = k + rem;
		ans = modAdd(ans, store[index]);
		n /= 10;
