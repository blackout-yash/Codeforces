// D. Radio Towers
// Link - https://codeforces.com/problemset/problem/1452/D
...
// f(n) = f(n-1) + f(n-3) + f(n-5) +....   (i)

// On replacing n by n-2, we get;
// f(n-2) = f(n-3) + f(n-5) + f(n-7) +....  (ii)

// From eq (i) & (ii), we get:
// f(n) = f(n-1) + f(n-2)

// Link - https://codeforces.com/blog/entry/84847?#comment-724158
void solve() {
	/*-------__👌Never Give Up👌__-------*/
	...
	vector <int> dp(2);
	dp[1] = 1;
	fora(i, 1, n + 1) dp[i % 2] = modAdd(dp[i % 2], dp[1 - (i % 2)]);

	cout << modDivide(dp[n % 2], powMod(2, n));
}
